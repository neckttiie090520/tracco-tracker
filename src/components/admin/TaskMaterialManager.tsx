...
import type { TaskMaterial, CreateTaskMaterialRequest, DisplayMode } from '../../types/materials';
...
interface TaskMaterialManagerProps {
  taskId: string;
...
interface AddTaskMaterialModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (material: CreateTaskMaterialRequest) => void;
  isLoading: boolean;
}
...
function AddTaskMaterialModal({ isOpen, onClose, onAdd, isLoading }: AddTaskMaterialModalProps) {
  ...
  const getPreviewMaterial = useCallback(() => {
    ...
    // Convert TaskMaterial to WorkshopMaterial for preview (they have same display structure)
    return {
      id: 'preview',
      workshop_id: 'preview', // WorkshopMaterialDisplay expects workshop_id
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (!url.trim() || !validationResult?.valid) {
      console.log('Invalid material, not adding');
      return;
    }
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-gray-900">Add Task Material</h2>
            <button
              onClick={handleClose}
...
export function TaskMaterialManager({ taskId, materials, onMaterialsChange, className = '' }: TaskMaterialManagerProps) {
  ...
  const handleAddMaterial = async (materialRequest: CreateTaskMaterialRequest) => {
    setIsLoading(true);
    try {
      // Validate the material request
      if (!materialRequest.url || !materialRequest.url.trim()) {
        console.error('Cannot add material without URL');
        return;
      }
  ...
  return (
    <div className={className}>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-medium text-gray-900">Task Materials</h3>
        <button
          type="button"
      ...
      {materials.length === 0 ? (
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
          ...
          <h3 className="text-md font-medium text-gray-900 mb-2">No materials added</h3>
          <p className="text-gray-600 text-sm mb-4">Add reference materials like Google Docs, tutorials, or examples to help participants complete this task.</p>
          <button
            type="button"
            onClick={() => setShowAddModal(true)}
          ...
          {materials.map((material, index) => (
            <div key={material.id} className="border border-gray-200 rounded-lg p-4">
              ...
              <div className="border border-gray-100 rounded p-2 bg-gray-50">
                <WorkshopMaterialDisplay material={{
                  ...material,
                  workshop_id: 'preview' // WorkshopMaterialDisplay expects workshop_id
                }} />
              </div>
